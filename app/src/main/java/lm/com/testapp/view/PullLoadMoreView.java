...
public class PullLoadMoreView extends LinearLayout {
    ...
    public ViewGroup addHeadView(int resLayout) {
        ...
        //测量头部布局高度
        headLayout.measure(MeasureSpec.AT_MOST,0);
        int headHeight = headLayout.getMeasuredHeight() + 50;
        headLayout.setVisibility(INVISIBLE);
        buildView(headHeight);
        return headLayout;
    }
     ...
     * @param headHeight
     */
    private void buildView(final int headHeight) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN://按下
                ...
                break;
            case MotionEvent.ACTION_MOVE://滑动
                scrollYValue = (event.getY() - mLastY);//手指滑动的上下距离
                float abs = Math.abs(scrollYValue);//转换为正值
                        ...
                        if (nestedScrollView.getScrollY() == 0) {
                            if (stateView == VIewState.CLOSE) {
                                if (mPaddingTop < 0) {//向下滑动但是头部空间没完全显示
                                    mPaddingTop = (int) (decayRatio * scrollYValue - subViewHeight);//这是头部布局距顶部的paddingtop值
                                    topLayout.setPadding(getPaddingLeft(), mPaddingTop, getPaddingRight(), mPaddingBottom);//重置头部布局padding
                                    stateMove = TouchStateMove.DOWN_NO_OVER;
                                    dotView.setPercent(1 - ((float) mPaddingTop / (-subViewHeight)));
                                    if (decayRatio * scrollYValue > 150) {//当滑动的距离大于150时,松开显示头部布局
                                        showToDown(headLayout, (long) 200);
                                        //设置状态为 松开抬起手时可以打开头部布局
                                        stateMove = TouchStateMove.DOWN_OVER;
                                    }
                                    //设置透明度
//                                    headLayout.setAlpha((float) (1 - ((float) -150 / (-decayRatio * scrollYValue))));
                                }
                            } else {
