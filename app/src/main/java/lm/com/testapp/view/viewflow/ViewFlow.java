...
import lm.com.testapp.adapter.YTBaseAdapter;
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private static final int DEFAULT_TIMESPAN = 5000;
	...
	// 是否自动滚动
	private boolean mIsAutoFlow;
	// 自动滚动时间间隔
	private int mAutoTimeSpan;
	// 滚动消息处理
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mIsAutoFlow = styledAttrs.getBoolean(R.styleable.ViewFlow_autoflow, true);
		mAutoTimeSpan = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, DEFAULT_TIMESPAN);
		init();
	}
	...
	public void setAutoFlow(boolean bAuto) {
		mIsAutoFlow = bAuto;
		startAutoFlow();
	}
	...
	private void startAutoFlow() {
		...
		handler.sendMessageDelayed(message, mAutoTimeSpan);
	}
	...
	public int getViewsCount() {
		if (mIsLoop) {
			if (mAdapter instanceof YTBaseAdapter) {
				YTBaseAdapter adapter = (YTBaseAdapter) mAdapter;
				return adapter.getItems().size();
			} else {
				throw new IllegalArgumentException("循环处理请继承YTBaseAdapter");
			}
		}
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator != null) {
			 ...
			 */
			int switchIndex = mIsLoop ? mCurrentAdapterIndex%getViewsCount() : mCurrentAdapterIndex;
			int hPerceived = h + (switchIndex - mCurrentBufferIndex)
					* getChildWidth();
	...
	public void setAdapter(Adapter adapter, int initialPosition) {
		...
		if (mAdapter.getCount() == Integer.MAX_VALUE) {
			mIsLoop = true;
		} else {
			mIsLoop = false;
		}
	...
	public void setSelection(int position) {
		...
		int switchIndex = mIsLoop ? mCurrentAdapterIndex%getViewsCount() : mCurrentAdapterIndex;
		if (mIndicator != null) {
			mIndicator.onSwitched(currentView, switchIndex);
		}
		if (mViewSwitchListener != null) {
			mViewSwitchListener.onSwitched(currentView, switchIndex);
		}
	}
...
//		Logs.d("viewflow", "Size of mLoadedViews: " + mLoadedViews.size() +
//				", Size of mRecycledViews: " + mRecycledViews.size() +
//				", X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
//		Logs.d("viewflow", "IndexInAdapter: " + mCurrentAdapterIndex
